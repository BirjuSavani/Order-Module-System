...
class CustomerController {
    ...
    async add(req, res) {
        try {
            ...
            const existingCustomer = await CustomerMaster.findOne({ where: { customer_number: { [Op.like]: `%${customer_number}%` } } });
            if (existingCustomer) {
                return res.status(409).json({ error: `Customer with this number ${customer_number} already exists. Please try a different number.` });
            }
    ...
    async updateOneCustomer(req, res) {
        ...
        try {
            ...
            return res.status(500).json({ error: "Can't Update Customer data." });
        }
    }
    ...
    async deleteCustomer(req, res) {
        try {
            ...
            // }
            return res.status(500).json({ error: "Can't Delete Customer" });
        }
    }
