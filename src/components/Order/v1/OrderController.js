...
class OrderController {
    ...
    async get(req, res) {
        ...
        try {
            ...
            return res.status(500).json({ error: "Can't Find order Data" });
        }
    }
        ...
        try {
            ...
            for (const product of cartDetail) {
                ...
                if (productDetail.stock < product.qty) {
                    return res.status(422).json({ message: `Product ID ${productDetail.id} ${productDetail.title} has only ${productDetail.stock} in stock.` })
                }
    ...
    async getOneOrder(req, res) {
        ...
        try {
            ...
            await transaction.rollback(); // Rollback the transaction in case of error
            return res.status(500).json({ message: "Can't Find particular Order Data" });
        }
    }
    ...
    async update_Cart(req, res) {
        ...
        try {
            ...
            console.log(cart_id)
            const orderplaced = await OrderMaster.findOne({ where: { cart_id } });
            if (orderplaced) {
                return res.status(404).json({ message: `The Cart with the ID '${cart_id}' Unable to update cart because is already placed order.` });
            };
    ...
    async deleteCartProduct(req, res) {
        ...
        try {
            ...
            return res.status(500).json({ message: "Can't Delete Cart's Product" });
        }
    }
    ...
    async deleteOrder(req, res) {
        ...
        try {
            ...
            // Check if Order is shipped or delivered in the order

            // const orderStatues = await OrderMaster.findOne({ where: { order_id, status: "delivered" } });
            // if (orderStatues.status === "delivered") {
            //     return res.status(404).json({ message: `The order with the ID '${order_id}' cannot be canceled because it has already been delivered.` });
            // };
            const orderStatuss = await OrderMaster.findOne({ where: { order_id, status: ["delivered"] } });
            if (orderStatuss) {
                const statusMessage = orderStatus.status === "" ? "" : "delivered";
                return res.status(404).json({ message: `The order with the ID '${order_id}' cannot be canceled because it has already been ${statusMessage}.` });
            }

            // Check if Order has been Canceld or not
